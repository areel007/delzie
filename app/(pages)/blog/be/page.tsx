...
export default function BE() {
    ...
    const fetchBEBlogs = async () => {
        try {
            const res = await API.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/blogs?category=be&page=${page}&limit=${limit}`);
    ...
    return (
        <>
            <InnerBlogLayout blogs={blogs} category="be" />
