...
export default function FE() {
    ...
    const fetchFEBlogs = async () => {
        try {
            const res = await API.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/blogs?category=fe&page=${page}&limit=${limit}`);
    ...
    return (
        <>
            <InnerBlogLayout blogs={blogs} category="fe" />
